...
import argparse
#主模型
from Training.model_self_learning_F01_phone_new import my_ac2art_model
#语音分解模型
from Training.model_learning import ac2art_model
#自监督模型
# from Training.model_self_learning import ac2art_model2
import torch
import os
import csv
...
def train_model(test_on, n_epochs, loss_train, patience, select_arti, corpus_to_train_on, batch_norma, filter_type,
    ...
    #加载音素识别的模型的参数
    # file_weights1 = "saved_models/F01_dep_Haskins_loss_90_filter_out_bn_False_3.pt"
    # loaded_state = torch.load(file_weights1,map_location=device) #加载模型的键值对
    ...
    #修改learning rateing ,在前半部分减半，再后边部分保持不变，第二步要测试的东西
    ...
    for epoch in range(n_epochs):
        ...
        for categ in categs_to_consider:
            ...
            #print(len(files_this_categ_courant))
