...
# from Training.model_learning_F01 import my_ac2art_model as SAFN
...
#         self.lamda = 0.5
#         self.l = 0.1
#         self.theta = math.log(self.lamda/self.l+1)
#         self.alpha = 0.2
...
#         self.conv3.weight = Parameter(self.theta * self.conv3.weight + (1-self.theta))
#         self.conv5 = nn.Conv1d(in_channels=128,out_channels=128,kernel_size=5,padding=2)
#         self.conv5.weight = Parameter(self.theta * self.conv5.weight + (1-self.theta))
#         self.conv7 = nn.Conv1d(in_channels=128,out_channels=128,kernel_size=7,padding=3)
#         self.conv7.weight = Parameter(self.theta * self.conv7.weight + (1-self.theta))
#         self.conv9 = nn.Conv1d(in_channels=128,out_channels=128,kernel_size=9,padding=4)
#         self.conv9.weight = Parameter(self.theta * self.conv9.weight + (1-self.theta))
...
class my_ac2art_model(torch.nn.Module):
    ...
    def __init__(self, hidden_dim, input_dim, output_dim, batch_size,name_file="", sampling_rate=100,cutoff=10,cuda_avail =False, filter_type=1, batch_norma=False):
        ...
        self.phone_extractor = ext(640, hidden_dim, 3, 12)
        ...
        self.CNN_1D = CNN_1D()
        self.phone_CNN_1D = phone_CNN_1D()
        #####
    ...
    def forward(self, x, filter_output=None):
        ...
        # print(speaker_idt.size())  4*300*100


        #首先phone进行特征提取，得到预测的ema坐标
        phone_out = self.phone_CNN_1D(phone);
        # print(phone_out.size())  4*640*300
        # 再将phone送入到lstm中，进行phone逆推
        phone_out = phone_out.permute(0, 2, 1)  #4*300*640
        y_all_fu = self.phone_extractor(phone_out)
        # print(y_all_fu.size())  4*300*12

        #对mfcc进行特征提取
        out = self.CNN_1D(acf)
        # print(out.size())    4*640*300
        out = out.permute(0,2,1)

        phone = phone.permute(0, 2, 1)
        Input = torch.cat((out,phone,speaker_idt,y_all_fu),2)
        # print(Input.size())  #4*300*(801+12)
        Input = self.fc1(Input)  #4*300*300
        y_fu = self.extractor(Input)  #4*300*6
        Input_new = torch.cat((Input,y_fu),2)  #4*300*(813+6)
    ...
    def plot_results(self, index = None, y_target = None, y_pred_smoothed=None, y_lstm= None, y_SAFN= None,to_cons=[]):
        """
        ...
        for j in idx_to_cons:
            ...
            plt.legend(["target", "SPN", "BLSTM"])
            plt.title("{0}_{1}_{2}.png".format(self.name_file, articulators[j],index))
            # if y_pred_not_smoothed is not None:
            #     plt.legend(["target", "pred smoothed", "pred not smoothed"])
            # else:
            #     plt.legend(["target", "pred smoothed"])
            save_pics_path = os.path.join(
                "./images/{0}_{1}_{2}.png".format(self.name_file, articulators[j],index))
            # print(save_pics_path)
    ...
    def evaluate_on_test(self, X_test, Y_test, phone, std_speaker, to_plot=False, to_consider=None, verbose=True, index_common = [], no_std = False):
        ...
        all_diff = np.zeros((1, 6))
        all_pearson = np.zeros((1, 6))
        # all_diff = np.zeros((1, 12))
        # all_pearson = np.zeros((1, 12))
        if to_plot:
            indices_to_plot = np.random.choice(len(X_test), len(X_test), replace=False)
        #语音分解预训练模型
        model2 = ac2art_model()
        ...
        # #加载SAFN模型
        # model4 = SAFN(3,357,6,1)
        # model4 = model4.double().cuda()
        # #file_weights = "saved_models/F01_spec_loss_90_filter_fix_bn_False_32.pt"
        # file_weights = "saved_models/F01_indep_Haskins_loss_90_filter_out_bn_False_21.pt"
        # loaded_state = torch.load(file_weights,map_location=self.device)
        # model4.load_state_dict(loaded_state)
        for i in range(len(X_test)):
            ...
            y_all_fu = y_all_fu.detach().numpy().reshape((L, 12))  # y_pred (L,6)
            if to_plot:
               if i in indices_to_plot:
                   self.plot_results(index = i, y_target = y, y_pred_smoothed = y_pred_smoothed,
                                     y_lstm = y_lstm, to_cons = to_consider)
            ...
            for k in range(self.output_dim):
                ...
                # pearson[k] = np.corrcoef(y[:, k].T, y_all_fu[:, k].T)[0, 1]
