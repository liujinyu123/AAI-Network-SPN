...
from Training.tools_learning import which_speakers_to_train_on, give_me_train_valid_test_filenames, \
    cpuStats, memReport, criterion_both_zhen, load_np_ema_and_mfcc_and_phone, plot_filtre, criterion_pearson
import json
    ...
    for epoch in range(n_epochs):
        ...
        for categ in categs_to_consider:
            ...
            while len(files_this_categ_courant) > 0: # go through all  the files batch by batch
                ...
                loss = criterion_both_zhen(y1, y_pred, y2, y_fu, cuda_avail = cuda_avail, device=device)
                loss.backward()
                optimizer.step()
        ...
        if epoch >= 0 :
            ...
            rmse_per_arti_mean, pearson_per_arti_mean = model.evaluate_on_test(x, y, phone,  std_speaker = std_speaker, to_plot=True
